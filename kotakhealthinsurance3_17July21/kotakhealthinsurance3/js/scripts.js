    ...
    if ($('body').hasClass('family-2-1')) {
        ...
        if (insured_qty_string != null) {
            ...
            for (var x = 0; x <= insured_qty.length - 1; x++) {
                ...
                if (insured_qty[x].indexOf('boys') != -1) {
                      var res = insured_qty[x].split("_");
                      var boys_cou =  parseInt(res[1])  
                      var child_counts = parseInt(res[1]); 
                    for (var s = 1; s <= child_counts; s++) {
                        $('.child_' + s + '_container').show()
                        $('.child_' + s + '_container').find('label').text("Dependent Son "+s)
                        $('[name="child_' + s + '_age"]').attr('rel', 'active')
                        // $('.childrenNumber').find('.sd').html('Son');
                    }
                }
                if (insured_qty[x].indexOf('girls') != -1) {
                //    var boys_cou =  parseInt(insured_qty[2].split("_")[1])
                    var res = insured_qty[x].split("_");
                    var b_count   =  parseInt(boys_cou === undefined ?  0 : boys_cou)
                    var child_counts = (parseInt(res[1]) + b_count); 
                    count = 1
                  for (var d = (1+ b_count); d <= child_counts; d++) {
                      $('.child_' + d + '_container').show()
                      $('.child_' + d + '_container').find('label').text("Dependent Daughter "+ count)
                      $('[name="child_' + d + '_age"]').attr('rel', 'active')
                    //   $('.childrenNumber').find('.sd').html('Daughter');
                      count ++
                  }
              }  
            }
